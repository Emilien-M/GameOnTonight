...
public interface IBoardGameRepository : IRepository<BoardGame>
{
    ...
    Task<IEnumerable<BoardGame>> FilterGamesAsync(int playerCount, int maxDuration, string? gameType);
    ...
    Task<BoardGame?> GetRandomGameAsync(IEnumerable<int> gameIds);
    
    /// <summary>
    /// Retrieves the list of distinct game types for a user.
    /// </summary>
    /// <returns>List of game types.</returns>
    Task<IEnumerable<string>> GetDistinctGameTypesAsync();
}
